      ...
      <div style={{ flex: 1, display: 'flex', gap: 0, minHeight: 0 }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '16px 18px', gap: 14 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0, border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden', boxShadow: '0 2px 4px -1px rgba(0,0,0,0.03)' }}>
              {M3_ECONOMICS.map((row, i) => (
                <div key={i} style={{
                  ...
                  padding: '8px 12px',
                  background: row.highlight ? `${COLOR}10` : '#ffffff',
                  borderBottom: i < M3_ECONOMICS.length - 1 ? '1px solid #f1f5f9' : 'none',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <span style={{ fontFamily: 'Outfit, sans-serif', fontSize: 16, fontWeight: 700, color: row.highlight ? COLOR : '#374151', minWidth: 50, textAlign: 'right' }}>{row.value}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Inter, sans-serif', fontSize: 10, border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
                  <th style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Volume (Annual)</th>
                  <th style={{ padding: '6px 8px', textAlign: 'right', fontWeight: 600, color: '#374151' }}>Additive Cost</th>
                  <th style={{ padding: '6px 8px', textAlign: 'right', fontWeight: 600, color: '#374151' }}>Cement Savings</th>
                  <th style={{ padding: '6px 8px', textAlign: 'right', fontWeight: 600, color: '#374151' }}>Net Benefit</th>
                  <th style={{ padding: '6px 8px', textAlign: 'center', fontWeight: 600, color: '#374151' }}>CO₂ Saved</th>
                </tr>
              </thead>
              <tbody>
                {SCALE_TABLE.map((row, i) => (
                  <tr key={i} style={{ borderBottom: i < SCALE_TABLE.length - 1 ? '1px solid #f1f5f9' : 'none' }}>
                    ...
                    <td style={{ padding: '6px 8px', textAlign: 'center' }}>
                      <span style={{ background: 'rgba(52,211,153,0.15)', border: '1px solid rgba(52,211,153,0.3)', color: '#6ee7b7', padding: '3px 8px', borderRadius: 12, fontSize: 9, fontWeight: 700 }}>{row.co2Saved}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div style={{
          ...
          <div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Inter, sans-serif', fontSize: 9.5, border: '1px solid #e5e7eb', borderRadius: 12, overflow: 'hidden' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid #e5e7eb' }}>
                  <th style={{ padding: '5px 7px', textAlign: 'left', fontWeight: 600, color: '#374151' }}>Scenario</th>
                  <th style={{ padding: '5px 7px', textAlign: 'right', fontWeight: 600, color: '#374151' }}>CO₂</th>
                </tr>
              </thead>
              <tbody>
                {CO2_TABLE.map((row, i) => (
                  <tr key={i} style={{
                    background: row.highlight ? '#d1fae580' : '#ffffff',
                    borderBottom: i < CO2_TABLE.length - 1 ? '1px solid #f1f5f9' : 'none',
          ...
          <div style={{ background: COLOR, borderRadius: 12, padding: '14px', textAlign: 'center', color: '#fff', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
            <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: 13, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 8, opacity: 0.8 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-around' }}>
              {[
                ...
                <div key={item.label} style={{ textAlign: 'center' }}>
                  <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: 22, fontWeight: 700, lineHeight: 1 }}>{item.val}</div>
          ...
          <div style={{ background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: 12, padding: '10px 12px', boxShadow: '0 2px 4px -1px rgba(0,0,0,0.03)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
              ...
              <div style={{ fontFamily: 'Outfit, sans-serif', fontSize: 11, fontWeight: 700, color: '#374151', letterSpacing: 1, textTransform: 'uppercase' }}>
                Certifications
              </div>
            </div>
