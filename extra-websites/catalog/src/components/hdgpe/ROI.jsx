    ...
    <div className="page page-break" style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', gap: 0, minHeight: 0 }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Inter, sans-serif', fontSize: 10 }}>
              <thead>
                <tr style={{ background: COLOR, color: '#fff' }}>
                  <th style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 600 }}>Application</th>
                  <th style={{ padding: '6px 8px', textAlign: 'right', fontWeight: 600 }}>Gross Benefit</th>
                  <th style={{ padding: '6px 8px', textAlign: 'right', fontWeight: 600 }}>Additive Cost</th>
                  <th style={{ padding: '6px 8px', textAlign: 'right', fontWeight: 600 }}>Net Profit/ton</th>
                  <th style={{ padding: '6px 8px', textAlign: 'left', fontWeight: 600 }}>Driver</th>
                </tr>
              </thead>
              <tbody>
                {APP_ROI.map((row, i) => (
                  <tr key={i} style={{
                    background: row.highlight ? `${COLOR}10` : i % 2 === 0 ? '#fff' : '#f0fdff',
              ...
              <thead>
                <tr style={{ background: '#f3f4f6' }}>
                  {['Volume/yr', 'HD-G-PE Cost', 'Net Benefit', 'ROI', 'Notes'].map((h, i) => (
                    <th key={i} style={{ padding: '5px 8px', textAlign: i < 2 ? 'left' : i < 4 ? 'right' : 'center', fontWeight: 600, color: '#374151', borderBottom: '2px solid #e5e7eb' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {VOLUME_SCALE.map((row, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? '#fff' : '#fafafa' }}>
                    ...
                    <td style={{ padding: '5px 8px', textAlign: 'right' }}>
                      <span style={{ background: `${COLOR}15`, color: COLOR, padding: '1px 7px', borderRadius: 10, fontSize: 9, fontWeight: 700 }}>{row.roi}</span>
                    </td>
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 0, border: '1px solid #e5e7eb', borderRadius: 4, overflow: 'hidden' }}>
              {FORM_ADVANTAGES.map((row, i) => (
                <div key={i} style={{
                  ...
                  borderBottom: i < FORM_ADVANTAGES.length - 1 ? '1px solid #e5e7eb' : 'none',
          ...
          <div>
            <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 11, fontWeight: 700, color: '#111', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 8 }}>
          ...
          <div style={{ background: COLOR, borderRadius: 4, padding: '12px', textAlign: 'center', color: '#fff' }}>
            <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 13, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', opacity: 0.85, marginBottom: 6 }}>
              Additive Price
            </div>
            <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 26, fontWeight: 700, lineHeight: 1 }}>₹19,000</div>
