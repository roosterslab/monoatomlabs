...
const PRODUCT = { label: 'CERAPHENE', sub: 'Ceramic Car Coating', color: '#64748b', heroImage: '/images/ceraphene-01.jpg' }
const NAV = [
...
const SIDEBAR_W = 240
const PAGE_W_PX = 297 * (96 / 25.4)  // A4 landscape width at 96dpi
const PAGE_H_PX = 210 * (96 / 25.4)  // A4 landscape height at 96dpi
...
export default function App() {
  ...
  // Keep ref in sync with state (stable value for event handlers)
  useEffect(() => { userScaleRef.current = userScale }, [userScale])

  // Recompute fit scale on resize; reset user zoom on resize
  useEffect(() => {
    const update = () => {
  ...
  // Pinch-to-zoom + double-tap reset + Ctrl+wheel zoom
  useEffect(() => {
    ...
    const onTouchMove = (e) => {
      if (e.touches.length === 2 && pinchRef.current.active) {
        ...
        const next = Math.min(4, Math.max(0.25, pinchRef.current.startScale * ratio))
        setUserScale(next)
      }
    }
  ...
  return (
    <div id="app-root" style={{ display: 'flex', height: '100vh', fontFamily: 'Inter, sans-serif', position: 'relative' }}>
      ...
      {/* Zoom toolbar — fixed bottom center, always visible */}
