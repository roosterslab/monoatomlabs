  ...
  return (
    <div style={{ width: W, height: H, background: '#050505', position: 'relative', overflow: 'hidden', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: Math.max(18, Math.round(26 * s)), fontFamily: "'Outfit', sans-serif", color: '#ffffff', fontWeight: 800, letterSpacing: '0.08em', lineHeight: 1 }}>GRAFFISOL</div>
        <div style={{ fontSize: Math.max(6, Math.round(8 * s)), color: C, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Solar Panel Coating</div>
      </div>
      ...
      <div style={{
        ...
        {[{ val: '+10%', lab: 'Power' }, { val: '5-6°C', lab: 'Temperature' }, { val: 'Anti', lab: 'Soiling' }, { val: 'Hydro', lab: 'Phobic' }].map(({ val, lab }) => (
          <div key={val} style={{ flex: 1, background: 'linear-gradient(135deg, #1a0e00, #2a1800)', border: `1px solid ${C}`, borderRadius: Math.round(4 * s), display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: Math.round(2 * s) }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: Math.max(6, Math.round(9 * s)), color: '#ffffff', letterSpacing: '0.1em', fontStyle: 'italic' }}>Harness More. Lose Less.</div>
