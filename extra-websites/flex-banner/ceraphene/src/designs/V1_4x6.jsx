  ...
  return (
    <div style={{ width: W, height: H, background: '#050505', position: 'relative', overflow: 'hidden', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: Math.max(18, Math.round(26 * s)), fontFamily: "'Outfit', sans-serif", color: '#ffffff', fontWeight: 800, letterSpacing: '0.08em', lineHeight: 1 }}>CERAPHENE</div>
        <div style={{ fontSize: Math.max(6, Math.round(8 * s)), color: C, letterSpacing: '0.1em', textTransform: 'uppercase' }}>Ceramic Car Coating</div>
      </div>
      ...
      <div style={{
        ...
        {[{ val: '9H+', lab: 'Hardness' }, { val: '115°', lab: 'Contact Angle' }, { val: '4 YRS', lab: 'Durability' }, { val: 'NANO', lab: 'Technology' }].map(({ val, lab }) => (
          <div key={val} style={{ flex: 1, background: 'linear-gradient(135deg, #0f172a, #1e293b)', border: `1px solid ${C}`, borderRadius: Math.round(4 * s), display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: Math.round(2 * s) }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: Math.max(6, Math.round(9 * s)), color: '#ffffff', letterSpacing: '0.1em', fontStyle: 'italic' }}>Protect. Shine. Endure.</div>
