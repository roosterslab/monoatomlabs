...
const stats = [
  ...
  { value: '150+',   label: 'MW Coated' },
  { value: '6',      label: 'States Active' },
  { value: '18+',    label: 'Partner Firms' },
]

const cities = [
  { city: 'Jodhpur, Rajasthan', count: 8, note: 'Utility-scale desert solar' },
  { city: 'Kutch, Gujarat',     count: 6, note: 'Large hybrid parks' },
  { city: 'Anantapur, AP',      count: 5, note: 'South India solar corridor' },
  { city: 'Bangalore',          count: 4, note: 'Commercial rooftop' },
  { city: 'Pavagada, Karnataka',count: 3, note: 'Ultra Mega Solar Park' },
]

const international = [
  { city: 'Dubai, UAE', count: 2, note: 'High-temperature solar — dust & heat stress validation' },
]

export default function SolImpactMap() {
  ...
  return (
    <div style={{ background: '#050505', fontFamily: 'Inter, sans-serif', padding: '40px', maxWidth: 860, margin: '0 auto' }}>
      <div style={{ marginBottom: 28, borderBottom: '1px solid #1f1f1f', paddingBottom: 20 }}>
        <div style={{ fontSize: 11, letterSpacing: 4, color: C, fontWeight: 700, marginBottom: 6, textTransform: 'uppercase' }}>GRAFFISOL · Deployment</div>
        <div style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 36, fontWeight: 700, color: '#fff', letterSpacing: 2 }}>IMPACT MAP</div>
        <div style={{ color: '#475569', fontSize: 13, marginTop: 4 }}>Active solar installations across India — 6 states, 150+ MW, utility to rooftop</div>
      </div>
      ...
      <div style={{ background: '#0d0d0d', border: '1px solid #1f1f1f', borderRadius: 12, padding: 20, marginBottom: 16 }}>
        <div style={{ color: '#64748b', fontSize: 11, letterSpacing: 3, textTransform: 'uppercase', marginBottom: 16 }}>India — Active Solar Sites</div>
        {cities.map((row, i) => (
      ...
      <div style={{ marginTop: 4, padding: '8px 14px', background: '#1a1100', border: `1px solid ${C}33`, borderRadius: 8 }}>
        <span style={{ color: C, fontSize: 11 }}>🏛 Aligned with India's 500 GW renewable target — Graffisol adds 10–12% output to every coated MW</span>
      </div>

      <div style={{ marginTop: 16, textAlign: 'right', color: '#333', fontSize: 10, letterSpacing: 2 }}>MONOATOM LABS · GRAFFISOL · DEPLOYMENT MAP</div>
    </div>
  )
}
