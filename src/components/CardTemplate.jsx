...
const CardTemplate = ({ data, side = 'front' }) => {
  ...
  // Get border radius based on user preference
  const borderRadius = data.borderStyle === 'sharp' ? '0px' : '12px';
  const logoOpacity = data.logoOpacity ?? 0.8;
  const pad = {
    frontV: data.padFrontV ?? 0.875,
    frontH: data.padFrontH ?? 1.5,
    backV:  data.padBackV  ?? 0.875,
    backH:  data.padBackH  ?? 1.5,
  };
  const sc = {
    company: data.fsCompany || 1,
    tagline: data.fsTagline || 1,
    name:    data.fsName    || 1,
    role:    data.fsRole    || 1,
    contact: data.fsContact || 1,
    labels:  data.fsLabels  || 1,
    badges:  data.fsBadges  || 1,
  };
  ...
  const lsContact = data.lsContact ?? 0;
  const sideBarWidth = data.sideBarWidth ?? 12;
  const sectionGap = data.sectionGap ?? 0;
  const qrPadRight = data.qrPadRight ?? 0.75;
  const styles = getStyles(sc, pad, lsContact, sideBarWidth, sectionGap, qrPadRight);

  if (side === 'back') {
    return (
      <div style={{...styles.cardBack, borderRadius}}>
            ...
            <div style={styles.contactGrid}>
              <div style={styles.contactBlock}>
                ...
                <div style={styles.contactRow}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ flexShrink: 0 }}>
                    <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path>
                  </svg>
                  <span style={styles.contactValue}>{data.phone || '+91 851 197 6999'}</span>
                </div>
              </div>
              ...
              <div style={styles.contactBlock}>
                ...
                <div style={styles.contactRow}>
                  ...
                  <span style={styles.contactValue}>{data.email || 'ceo@monoatomlabs.com'}</span>
                </div>
              </div>
              ...
              <div style={styles.contactBlock}>
                ...
                <div style={styles.contactRow}>
                  ...
                  <span style={styles.contactValue}>{data.address || '3rd Floor, iHub, Ahmedabad - 380015'}</span>
                </div>
              </div>
            </div>

            {/* Footer - Recognition (Optional, Customizable) */}
            {data.showRecognition && data.recognitions && data.recognitions.length > 0 && data.recognitions.some(r => r.trim()) && (
              <div style={styles.recognition}>
                <p style={styles.contactLabel}>Recognition</p>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                  {data.recognitions.filter(r => r.trim()).map((recognition, index) => (
                    <span key={index} style={styles.badge}>{recognition}</span>
                  ))}
                </div>
              </div>
            )}
  ...
  // FRONT CARD
  return (
    <div className="card-front" style={{...styles.cardFront, borderRadius}}>
      {/* Canvas Background */}
...
const getStyles = (sc, pad, lsContact, sideBarWidth, sectionGap, qrPadRight) => ({
  // FRONT CARD STYLES
  cardFront: {
    width: '600px',
    height: '343px',
  ...
  contentLayer: {
    ...
    zIndex: 10,
    height: '100%',
    padding: `${pad.frontV}rem ${pad.frontH}rem`,
    display: 'flex',
  ...
  companyTitle: {
    ...
    fontWeight: 'bold',
    letterSpacing: '0.1em',
    fontSize: `${1.5 * sc.company}rem`,
    lineHeight: '1',
    margin: 0,
  },
  labsText: {
    color: 'white',
    fontSize: `${1 * sc.company}rem`,
  ...
  leftSection: {
    flex: '1.6',
    padding: `${pad.backV}rem ${pad.backH * 0.67}rem ${pad.backV}rem ${pad.backH}rem`,
    display: 'flex',
    flexDirection: 'column',
    justifyContent: 'space-between',
  ...
  contactValue: {
    fontSize: `${0.75 * sc.contact}rem`,
    fontWeight: 'bold',
    color: '#000',
    fontFamily: 'monospace',
    letterSpacing: `${lsContact}em`,
  },
  recognition: {
    marginTop: 'auto',
    paddingTop: '0.4rem',
  ...
  rightSection: {
    flex: '0.9',
    padding: `${pad.backV}rem ${qrPadRight}rem ${pad.backV}rem 1.25rem`,
    display: 'flex',
    flexDirection: 'column',
    alignItems: 'center',
